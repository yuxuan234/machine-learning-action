...
def plotDataSet(filename):
    ...
    for i in range(n):
        ...
        ycord.append(dataMat[i][2])  # 样本点 --第1列作为x轴数据，第2列作为y轴数据
    fig = plt.figure()
...
def chooseBestSplit(dataSet, leafType=regLeaf, errType=regErr, ops=(1, 4)):
    ...
    for featIndex in range(n - 1):
        ...
        for splitVal in set(dataSet[:, featIndex].T.A.tolist()[0]):
            # 根据特征和特征值切分数据集
            mat0, mat1 = tf.binSplitDataSet(dataSet, featIndex, splitVal)
            # 如果数据少于tolN,则退出
    ...
    if (S - bestS) < tolS:
        return None, leafType(dataSet)
    # 根据最佳的切分特征和特征值切分数据集合
    mat0, mat1 = tf.binSplitDataSet(dataSet, bestIndex, bestValue)
    # 如果切分出的数据集很小则退出
...
def createTree(dataSet, leafType = regLeaf, errType = regErr, ops = (1, 4)):
    ...
    retTree['spVal'] = val
    # 分成左数据集和右数据集
    lSet, rSet = tf.binSplitDataSet(dataSet, feat, val)
    # 创建左子树和右子树
